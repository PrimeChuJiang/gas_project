...
func _on_add_pressed() -> void:
	...
	tag_input.clear()
	refresh_tree_view()
	print("✅ 成功在内存大名单中追加标签: ", new_tag_str)
...
func _on_delete_pressed() -> void:
	...
	if not parent_tag_str.is_empty() and not is_parent_explicitly_registered:
		...
		print("ℹ️ 智能保护：已将父级标签 [", parent_tag_str, "] 转换为显式注册，防止其意外消失。")
	# -------------------- -------------------- --------------------

	# 5. 刷新 UI 树状视图
	refresh_tree_view()
	print("❌ 已从大名单数据中抹除目标项，共计移除行数: ", delete_count)
...
func _on_save_pressed() -> void:
	...
	# 4. 最终重绘 UI 树
	refresh_tree_view()
	print("💾 【双向同步】删除操作已完全固化到外部文本，且编辑器单例已安全重新加载！")
